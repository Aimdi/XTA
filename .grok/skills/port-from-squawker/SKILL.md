...
Guide for porting bug fixes or features from the Squawker codebase into XTA.
...
Read the full Squawker file before reading the XTA equivalent.

## Step 3 — Understand structural differences

XTA and Squawker share a common ancestor (Fritter) but have diverged. Key differences to check:
...
Do **not** blindly copy code. Adapt it to XTA's patterns.

## Step 4 — Port the change

- Apply the logic to the equivalent XTA file(s) in `lib/`.
...
- Preserve existing XTA variable names, style, and indentation — do not reformat unrelated code.
