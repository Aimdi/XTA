...
void main() {
  ...
  testWidgets(
    'real Home retains loaded readers and their selected sections when grouping is undone',
    (tester) async {
      ...
      if (const bool.fromEnvironment('RENDER_ALT_MICROBLOGGING')) {
        await expectLater(
          ...
          matchesGoldenFile(
            '../review-artifacts/renders/alt-microblogging-reader.png',
          ),
        );
      }
      ...
      expect(
        blue.client.calls,
        hasLength(1),
        reason: 'Opening actions must not refresh the feed.',
      );
      await tester.tap(
        find.widgetWithText(
      ...
      final picker = find.descendant(
        of: find.byType(PluginDockRow),
        matching: find.byType(PluginSectionPicker),
      );
      ...
      expect(
        blue.client.calls,
        hasLength(1),
        reason: 'Opening sections must not load another feed.',
      );
      await tester.tap(
        find.widgetWithText(
          PopupMenuItem<int>,
          L10n.current.plugin_bluesky_liked,
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.byKey(const ValueKey('home-source-mastodon')),
        findsOneWidget,
      );
      ...
      await tester.pumpAndSettle();
      expect(tester.state(find.byType(BlueskyScreen)), same(readerState));
      final mastodonChip = find.byKey(
        const ValueKey('alt-microblogging-service-mastodon'),
      );
      ...
      await tester.pumpAndSettle();
      final blueChip = find.byKey(
        const ValueKey('alt-microblogging-service-bluesky'),
      );
