...
void main() {
  test('recent searches are bounded, deduplicated and isolated by network', () async {
    ...
    for (var i = 0; i < 12; i++) { await store.remember('mastodon', 'query-$i'); }
    ...
    await store.remove('mastodon', 'QUERY-11'); await store.destroy();
  ...
  testWidgets('empty Mastodon Discover shows followed people and live topics', (tester) async {
    ...
    await tester.pumpWidget(h.app(child: const Scaffold(body: MastodonDiscoverSearch(query: ''))));
    await tester.pumpAndSettle();
    expect(find.text('Maya Chen'), findsOneWidget); expect(find.text('#photography'), findsOneWidget);
    expect(h.client.searches, 0); expect(tester.takeException(), isNull);
    await h.close(tester);
  });

  testWidgets('inline search keeps its results after opening a profile', (tester) async {
    final h = MastodonHarness();
    await tester.pumpWidget(h.app(child: const Scaffold(body: MastodonDiscoverSearch(query: 'design'))));
    await tester.pumpAndSettle();
    expect(find.byType(TextField), findsNothing);
    await tester.tap(find.text('Maya Chen')); await tester.pumpAndSettle();
    expect(find.byType(MastodonProfileScreen), findsOneWidget);
    await tester.pageBack(); await tester.pumpAndSettle();
    expect(find.text('Maya Chen'), findsOneWidget); expect(h.client.searches, 1);
