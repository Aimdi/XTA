...
const archivedPost = MastodonPost(id: '123', acct: 'artist@studio.example', authorName: 'Artist',
  text: 'Sketches for the garden', url: 'https://studio.example/@artist/123', spoilerText: 'Spoiler',
  sensitive: true, images: ['https://studio.example/garden.png']);
...
void main() {
  test('same status id from two servers never collides in Saved', () {
    const remote = MastodonPost(id: '123', acct: 'artist@other.example', authorName: 'Artist',
      text: 'Different', url: 'https://other.example/@artist/123');
  ...
  test('the shared index retains warning, author and searchable text', () {
    ...
    expect(content.mastodon!.sensitive, isTrue); expect(content.tweet, isNull); expect(content.reddit, isNull);
    expect(content.matches('garden'), isTrue); expect(content.matches('artist@studio'), isTrue);
  ...
  testWidgets('bookmark writes the shared archive and can undo it', (tester) async {
    ...
    final saved = MemorySaved(); final folders = SavedTweetFolderModel();
    await tester.pumpWidget(h.app(child: MultiProvider(providers: [
    ...
    await tester.tap(find.byIcon(Icons.bookmark)); await tester.pumpAndSettle();
    expect(saved.state, isEmpty); expect(tester.takeException(), isNull);
    await h.close(tester); await saved.destroy(); await folders.destroy();
  });
}
