...
const _mastodon = MastodonPost(id: '10', acct: 'alice@example.social', authorName: 'Alice', text: 'Post', url: 'https://example.social/@alice/10');
...
void main() {
  test('Bluesky reads repost actors and quote posts with opaque paging cursors', () async {
    ...
    final client = BlueskyClient(httpClient: MockClient((request) async {
      ...
      if (request.url.path.endsWith('getRepostedBy')) {
        return http.Response(jsonEncode({'repostedBy': [{'did': 'did:plc:b', 'handle': 'bob.test', 'displayName': 'Bob'}, {}], 'cursor': 'opaque-next'}), 200);
      }
      return http.Response(jsonEncode({'posts': [{
        'uri': 'at://did:plc:b/app.bsky.feed.post/quote', 'cid': 'cid',
  ...
  test('Bluesky retains stable repost identity when a handle is absent', () {
    final post = blueskyPostFromFeedItem({
      'post': {'uri': _uri, 'author': {'did': 'did:plc:author', 'handle': 'alice.test'}, 'record': {'text': 'Hello'}},
      'reason': {r'$type': 'app.bsky.feed.defs#reasonRepost', 'by': {'did': 'did:plc:b', 'displayName': 'Bob'}},
    })!;
  ...
  test('Mastodon uses Link cursor rather than the last account ID', () async {
    final calls = <Uri>[];
    final client = MastodonClient(httpClient: MockClient((request) async {
      calls.add(request.url);
      if (request.url.path == '/api/v1/statuses/10') {
        return http.Response(jsonEncode({'id': '10', 'url': _mastodon.url, 'content': '<p>Post</p>', 'account': {'id': '1', 'acct': 'alice'}}), 200);
      }
      return http.Response(jsonEncode([{'id': '999', 'acct': 'bob@elsewhere.test', 'display_name': 'Bob'}]), 200, headers: {
  ...
  test('Mastodon unauthorized quote lists stay errors rather than empty results', () async {
    ...
    await expectLater(client.getQuotes(['https://example.social'], _mastodon), throwsA(
      isA<MastodonException>().having((error) => error.kind, 'kind', MastodonErrorKind.unauthorized)));
  });
  ...
  test('Activity pagination rejects another server and endpoint', () {
    ...
    expect(mastodonActivityNextPage('<https://other.test/api/v1/statuses/10/reblogged_by>; rel="next"', current), isNull);
