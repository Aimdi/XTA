...
DiscoveryAccount account(String id, {
  DiscoverySource source = DiscoverySource.x,
  String? handle, String name = '', String text = '',
}) => DiscoveryAccount(source: source, id: id, handle: handle ?? id,
  name: name, text: text, postUrl: 'https://example.test/post/$id');

BlueskyPost sky(String id, {bool repost = false, BlueskyPost? quote}) =>
  BlueskyPost(uri: 'at://$id/app.bsky.feed.post/p', cid: 'cid',
    did: id, handle: '$id.bsky.social', authorName: id,
    text: 'A real post', url: 'https://bsky.app/profile/$id/post/p',
    repostedByHandle: repost ? 'group.member' : null, quotedPost: quote,
  );
...
void main() {
  ...
  test('deduplicates identities per network and excludes followed aliases', () {
    final ranked = rankDiscoveryAccounts([
      ...
      account('2', name: 'Space'), account('2'),
      account('2', source: DiscoverySource.mastodon),
      account('did:plc:member', source: DiscoverySource.bluesky,
        handle: 'member.bsky.social'),
    ], followed: {'x:followed', 'bluesky:member.bsky.social'}, groupName: 'Space');
  ...
  test('rejects invented AI ids and preserves candidates omitted by AI', () {
    ...
    final result = parseDiscoveryRanking(
      '{"ids":["x:2","invented:9","x:2"]}', candidates);
  ...
  test('Mastodon candidates come from boosts and quoted authors', () {
    const direct = MastodonPost(id: '1', acct: 'member@server.test',
      authorName: 'Member', text: 'Direct', url: 'https://server.test/1');
    const boost = MastodonPost(id: '2', acct: 'other@server.test',
      authorName: 'Other', text: 'Boost', url: 'https://server.test/2', boosted: true);
  ...
  test('source failure preserves candidates and normal discovery never uses AI', () async {
    var aiCalls = 0;
    final store = GroupDiscoveryStore(chat: (_, _) async {
      aiCalls++;
      return '{"ids":["x:1"]}';
    });
    addTearDown(store.destroy);
    await store.load(sources: [() async => [account('1')], () async => throw StateError('offline')],
      followed: {}, groupName: 'Space');
  ...
  test('unusable AI response keeps verified local candidates and reports fallback', () async {
    ...
    addTearDown(store.destroy);
    await store.load(sources: [() async => [account('1')]],
      followed: {}, groupName: 'Space', ai: ai);
    expect(store.state.accounts.single.id, '1');
  ...
  test('all-followed candidates never trigger the configured AI', () async {
    var calls = 0;
    final store = GroupDiscoveryStore(chat: (_, _) async { calls++; return ''; });
    addTearDown(store.destroy);
    await store.load(sources: [() async => [account('1')]],
      followed: {'x:1'}, groupName: 'Space', ai: ai);
  ...
  test('following from a profile removes its cached discovery card without refetch', () async {
    ...
    addTearDown(store.destroy);
    await store.load(sources: [() async {
      loads++;
  ...
  test('a superseded source request cannot call AI or replace newer results', () async {
    ...
    addTearDown(store.destroy);
    final first = store.load(sources: [() => older.future],
      followed: {}, groupName: 'Old', ai: ai);
    final second = store.load(sources: [() => newer.future],
      followed: {}, groupName: 'New');
  ...
  test('a superseded AI response cannot replace newer discovery', () async {
    ...
    final store = GroupDiscoveryStore(chat: (_, _) {
      started.complete();
      return reply.future;
    });
    addTearDown(store.destroy);
    final first = store.load(sources: [() async => [account('old')]],
      followed: {}, groupName: 'Old', ai: ai);
    await started.future;
    await store.load(sources: [() async => [account('new')]],
      followed: {}, groupName: 'New');
  ...
  test('following during a source request excludes its eventual result', () async {
    ...
    final store = GroupDiscoveryStore();
    addTearDown(store.destroy);
    final loading = store.load(sources: [() => response.future],
      followed: {}, groupName: 'Space');
