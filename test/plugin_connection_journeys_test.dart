...
class _Probe {
  ...
  Future<void> run() async { calls++; await response.future; }
}
class _Karakeep extends KarakeepClient {
  final _Probe probe;
  _Karakeep(this.probe);
  @override
  Future<bool> verify({required String baseUrl, required String apiKey}) async { await probe.run(); return true; }
}
...
class _Deepmarks extends DeepmarksClient {
  ...
  @override
  Future<String?> verify({required String baseUrl, required String apiKey}) async { await probe.run(); return null; }
}
  ...
  for (final id in ['karakeep', 'deepmarks', 'immich']) {
    testWidgets('$id settings: large text, reachable test/save, stale probe invalidation', (tester) async {
      ...
      final prefs = PrefServiceCache(defaults: {
        ...
        optionPluginImmichServerUrl: 'https://example.test', optionPluginImmichApiKey: 'test-key',
        optionPluginImmichAlbumPerFolder: false, optionPluginImmichIncludeVideos: false,
      ...
      await tester.pumpWidget(PrefService(service: prefs, child: MultiProvider(providers: [
        ...
        builder: (context, child) => MediaQuery(data: MediaQuery.of(context).copyWith(
          textScaler: TextScaler.linear(1.6)), child: child!),
        ...
        home: Scaffold(body: Builder(builder: (context) => TextButton(
          onPressed: () => Navigator.push(context, MaterialPageRoute(
            builder: (context) => pluginById(id)!.settingsScreen(context)!)),
