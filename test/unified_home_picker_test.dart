...
void main() {
  testWidgets(
    'ordinary source rows use at most 56px without shrinking targets',
    (tester) async {
  ...
  testWidgets('large chooser searches locally and preserves Add', (
    tester,
  ) async {
  ...
  testWidgets(
    'search matches grouped services and clearing restores category',
    (tester) async {
      const sources = [
        'bluesky',
        'mastodon',
      ...
      await tester.pumpAndSettle();
      expect(
        find.byKey(const ValueKey('home-source-alt-microblogging')),
        findsOneWidget,
      );
      ...
      await tester.tap(
        find.byTooltip(L10n.current.plugin_mastodon_clear_search),
      );
      await tester.pumpAndSettle();
      expect(
        find.byKey(const ValueKey('home-source-alt-microblogging')),
