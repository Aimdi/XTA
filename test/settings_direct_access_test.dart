...
Map<String, dynamic> readerMediaPreferences() => {
  ...
  optionMediaVideoPrefetchSeconds: 5,
...
void main() {
  testWidgets('search opens and highlights autoplay below the fold, and the toggle works', (tester) async {
    ...
    await tester.pumpWidget(h.app(child: const SettingsScreen()));
    await tester.pumpAndSettle();
    await tester.enterText(find.byType(TextField), 'autoplay');
    await tester.pumpAndSettle();
    ...
    expect(focus.hitTestable(), findsOneWidget);
  ...
  testWidgets('translated control index includes reading position and text size', (tester) async {
    ...
    expect(
      searchSettingsControls(l10n, 'reading position').any((result) => result.target == optionFeedReadingPosition),
      isTrue,
    );
  ...
  testWidgets('installed Mastodon has separate client and settings actions', (tester) async {
    ...
    await tester.pumpWidget(
      h.app(
        child: Scaffold(
          body: InstalledPluginRow(plugin: MastodonPlugin(), onUninstall: () {}, onChanged: () {}),
        ),
      ),
    );
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const ValueKey('plugin-open-mastodon')));
    await tester.pumpAndSettle();
    expect(find.byType(MastodonScreen), findsOneWidget);
    expect(find.byType(MastodonSettingsScreen), findsNothing);
    await tester.pageBack();
    await tester.pumpAndSettle();
    await tester.tap(find.byIcon(Icons.tune));
    await tester.pumpAndSettle();
    expect(find.byType(MastodonSettingsScreen), findsOneWidget);
    expect(find.byType(SettingsControlTarget), findsNothing);
    expect(tester.takeException(), isNull);
    await h.close(tester);
  });
}
