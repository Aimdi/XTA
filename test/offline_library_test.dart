...
final _png = base64Decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScLbtAAAAABJRU5ErkJggg==');
OfflineArticle _article({String body = '<p>A retained article</p>', String feed = 'example'}) =>
  OfflineArticle.rss(RssItem(id: '1', title: 'Reading in the city', feedId: feed,
    feedTitle: 'Daily journal', link: 'https://example.com/read/1', bodyHtml: body));
...
void main() {
  ...
  setUp(() async { directory = await Directory.systemTemp.createTemp('xta-offline-test-'); });
  tearDown(() async {
    for (final store in stores) { await store.destroy(); }
    stores.clear();
    if (await directory.exists()) await directory.delete(recursive: true);
  });
  OfflineStore store({http.Client? client, int storageLimit = OfflineStore.defaultStorageLimit,
    int imageLimit = 8 * 1024 * 1024}) {
    final model = OfflineStore(directory: () async => directory, client: client ?? MockClient((_) async => http.Response.bytes(_png, 200)),
      storageLimit: storageLimit, imageLimit: imageLimit);
    stores.add(model);
    return model;
  }

  test('article snapshots preserve identity, body, author and media metadata', () {
    final rss = OfflineArticle.rss(RssItem(id: '1', title: 'News', feedId: 'feed', feedTitle: 'Journal',
      author: 'Maya', bodyHtml: '<p>Body</p>', categories: ['Science'], publishedAt: DateTime.utc(2026, 9, 7)));
    ...
    const post = SubstackPost(id: '12', title: 'An episode', slug: 'episode', publicationBaseUrl: 'https://journal.substack.com',
      publicationName: 'Journal', bodyHtml: '<p>Full text</p>', authorName: 'Maya', audience: 'only_paid',
      audioUrl: 'https://example.com/audio.mp3', hasVideoUpload: true);
  ...
  test('cold restart reports exact partial images and renders without remote fetches', () async {
    var calls = 0;
    final model = store(client: MockClient((request) async {
      calls++;
    ...
    final rendered = await cold.renderArticle(article.id,
  ...
  test('Substack slug links find numeric-ID retained articles without a fetch', () async {
    final model = store();
    const post = SubstackPost(id: '123', title: 'Story', slug: 'a-story', publicationBaseUrl: 'https://journal.substack.com',
      publicationName: 'Journal', bodyHtml: '<p>Retained body</p>');
    ...
    final retained = await cold.article('substack:journal:a-story', canonicalUrl: 'https://journal.substack.com/p/a-story#comments');
  ...
  test('storage and per-image limits apply before publication', () async {
    ...
    await model.keepMedia(id: 'saved:1', title: 'Photo', source: 'Maya', media: const [OfflineMediaSource('https://example.com/a.png')]);
  ...
  test('streamed size overflow cannot create an available file', () async {
    final model = store(imageLimit: 32, client: _StreamClient((_) async =>
      http.StreamedResponse(Stream.fromIterable([_png.sublist(0, 16), _png.sublist(16)]), 200)));
    await model.keepMedia(id: 'saved:1', title: 'Photo', source: 'Maya', media: const [OfflineMediaSource('https://example.com/a.png')]);
    expect(model.state.entries, isEmpty);
  ...
  test('interrupted response cleans staging and is never available', () async {
    final model = store(client: _StreamClient((_) async => http.StreamedResponse(
      Stream<List<int>>.multi((events) { events.add(_png.sublist(0, 16)); events.addError(const SocketException('Interrupted')); events.close(); }), 200)));
    await model.keepMedia(id: 'saved:1', title: 'Photo', source: 'Maya', media: const [OfflineMediaSource('https://example.com/a.png')]);
    expect(model.state.entries, isEmpty);
  ...
  test('remove cancels an in-flight copy before publication', () async {
    ...
    final model = store(client: _StreamClient((_) async { started.complete(); return http.StreamedResponse(stream.stream, 200); }));
    final keeping = model.keepMedia(id: 'saved:1', title: 'Photo', source: 'Maya', media: const [OfflineMediaSource('https://example.com/a.png')]);
