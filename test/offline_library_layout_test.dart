...
List<OfflineEntry> _entries() => [
  OfflineEntry(
    id: 'rss:journal:1',
    directory: '${'a' * 64}-1',
    title: 'A slower way to read the city',
    source: 'The Daily Journal',
    kind: OfflineKind.article,
    totalMedia: 3,
    articleBytes: 42000,
    retainedAt: DateTime(2026, 9, 7),
    files: List.generate(
      3,
      (i) => OfflineFile(name: 'media-$i', url: 'https://example.com/$i.jpg', mime: 'image/jpeg', bytes: 240000),
    ),
    url: 'https://example.com/journal',
  ),
  OfflineEntry(
    id: 'substack:design:2',
    directory: '${'b' * 64}-2',
    title: 'What makes a place worth staying in?',
    ...
    retainedAt: DateTime(2026, 9, 7),
    files: const [
      OfflineFile(name: 'media-0', url: 'https://example.com/place.jpg', mime: 'image/jpeg', bytes: 680000),
    ],
  ...
  OfflineEntry(
    id: 'saved:3',
    directory: '${'c' * 64}-3',
    title: 'Morning light, just outside the city.',
    source: 'maya@studio.example',
    kind: OfflineKind.media,
    totalMedia: 2,
    retainedAt: DateTime(2026, 9, 6),
    files: List.generate(
      2,
      (i) => OfflineFile(name: 'media-$i', url: 'https://example.com/photo-$i.jpg', mime: 'image/jpeg', bytes: 1200000),
    ),
  ),
  OfflineEntry(
    id: 'saved:4',
    directory: '${'d' * 64}-4',
    title: 'A quiet moment from the archive',
    ...
    sensitive: true,
    files: const [
      OfflineFile(name: 'media-0', url: 'https://example.com/hidden.jpg', mime: 'image/jpeg', bytes: 720000),
    ],
  ),
];
...
void main() {
  ...
  for (final variant in ['offline-library', 'offline-library-dark', 'offline-library-large']) {
    testWidgets('offline production collection $variant', (tester) async {
        ...
        MaterialApp(
          theme: ThemeData(
            ...
            colorScheme: ColorScheme.fromSeed(
              seedColor: const Color(0xff49796b),
              brightness: dark ? Brightness.dark : Brightness.light,
            ),
          ),
          ...
          home: RepaintBoundary(
            key: const ValueKey('offline-window'),
            child: OfflineLibraryScreen(store: model),
          ),
        ),
      );
      ...
      await expectLater(
        find.byKey(const ValueKey('offline-window')),
