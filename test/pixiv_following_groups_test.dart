...
void main() {
  test('following picker loads public pages then private follows without duplicate artists', () async {
    ...
    final requests = <Uri>[];
    final client = PixivClient(prefs, httpClient: MockClient((request) async {
      ...
      return http.Response(jsonEncode({
        'user_previews': [
          {'user': {'id': index == 3 ? 1 : index, 'name': 'Artist $index', 'account': 'artist', 'is_followed': true}},
          {'user': {'id': 0}},
        ],
        if (index == 1) 'next_url': 'https://app-api.pixiv.net/v1/user/following?user_id=7&restrict=public&offset=30',
      }), 200);
    }));
  ...
  test('refresh discards an older following page and preserves its own cursor', () async {
    ...
    final requests = <Uri>[];
    http.Response page(int id, {String? next}) => http.Response(jsonEncode({
      'user_previews': [{'user': {'id': id, 'name': 'Artist', 'account': 'artist'}}],
      if (next != null) 'next_url': next,
    }), 200);
    final client = PixivClient(prefs, httpClient: MockClient((request) async {
      requests.add(request.url);
      if (requests.length == 1) return page(1, next: 'https://app-api.pixiv.net/v1/user/following?offset=30');
      if (requests.length == 2) { oldStarted.complete(); return oldPage.future; }
