    ...
    for (final direction in [TextDirection.ltr, TextDirection.rtl]) {
      for (final embedded in [true, false]) {
        testWidgets('reader controls fit $tokens $direction embedded=$embedded at 320dp and 200% text', (tester) async {
          ...
          var settings = 0;
          var selected = 0;
          final chrome = PluginHomeChrome(title: 'A long localized plugin name',
            mark: const Icon(Icons.rss_feed),
            tabs: [
              PluginHomeTab(label: 'Home', icon: Icons.home_outlined, selected: true, onTap: () => selected++),
              PluginHomeTab(label: 'Subscriptions and publications', icon: Icons.article_outlined,
                selected: false, onTap: () {}),
            ],
            actions: [IconButton(tooltip: 'Settings', icon: const Icon(Icons.settings_outlined),
              onPressed: () => settings++)],
          );
