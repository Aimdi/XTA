...
MastodonPost reply(String id, String? parent) => MastodonPost(
  id: id,
  replyToId: parent,
...
class DelayedProfileClient extends MastodonFixtureClient {
  ...
  Future<List<MastodonPost>> getStatuses(
    ...
    if (onlyMedia) {
      mediaReads++;
      return Future.value([reply('media', null)]);
    }
    return page.future;
  }
}
...
void main() {
  testWidgets('a fresh client session restores the visible post without fetching a new page', (tester) async {
    ...
    final raw = first.prefs.get<String>(mastodonReadingPreference)!;
    final anchor = (jsonDecode(raw)['points']['client:1']['anchor']) as String;
    final card = find.byWidgetPredicate((widget) => widget is MastodonPostCard && widget.post.url == anchor);
    final previousTop = tester.getTopLeft(card).dy;
    expect(anchor, isNot(samplePosts.first.url));
    await first.close(tester);
    final second = MastodonHarness(populated: false);
    await second.prefs.set(mastodonReadingPreference, raw);
    await tester.pumpWidget(second.app());
    await tester.pumpAndSettle();
    expect(card, findsOneWidget);
    expect(tester.getTopLeft(card).dy, closeTo(previousTop, 1));
    expect(second.client.publicReads, 0);
  ...
  test('cyclic and duplicate replies cannot loop or duplicate a card', () {
    final thread = MastodonThread(
      status: reply('root', null),
      descendants: [reply('a', 'b'), reply('b', 'a'), reply('a', 'b'), reply('root', 'a')],
    );
  ...
  test('restart restores choice, content and anchor only for its server scope', () async {
    ...
    expect(restored.state.points['client:2']!.anchor, samplePosts[12].url);
  ...
  test('switching to media during posts pagination keeps both responses in their own tab', () async {
    ...
    expect(store.state.mediaSelected, isTrue);
    expect(client.mediaReads, 1);
    await store.destroy();
          ...
          body: SizedBox(
            ...
            child: MastodonMediaTile(
              post: MastodonPost(
                ...
                sensitive: true,
                images: ['https://one.example/private.png'],
              ),
            ),
          ),
        ),
