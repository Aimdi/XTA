...
class BlueskyReadingClient extends BlueskyClient {
  ...
  Future<BlueskyFeedPage> getAuthorFeed(String actor, {int limit = 20, String? cursor, String? filter}) async {
    ...
    if (filter == kBlueskyAuthorFeedMedia)
      return BlueskyFeedPage(
        posts: [for (var i = 0; i < 18; i++) bluePost('image-$i', media: true, sensitive: i == 3, video: i == 1)],
        cursor: 'media-next',
      );
    if (filter == kBlueskyAuthorFeedReplies)
      return BlueskyFeedPage(
        posts: [
          bluePost('not-a-reply'),
  ...
  Future<BlueskyThread> getPostThread(String uri, {int depth = 10, int parentHeight = 80}) async {
    ...
    return BlueskyThread(
      post: bluePost('root'),
      ancestors: [bluePost('context')],
      replies: [
        bluePost('a', parent: 'root'),
...
class BlueLikes extends BlueskyLikesStore {
  ...
  Future<void> toggle(BlueskyPost post) async =>
      update(isLiked(post.uri) ? state.where((liked) => liked.uri != post.uri).toList() : [post, ...state]);
}
    ...
    child: MultiProvider(
      ...
      child: MaterialApp(
        ...
        builder: (context, child) => RepaintBoundary(
          key: const ValueKey('bluesky-window'),
          child: MediaQuery(
