...
MastodonPost reviewPost(String id, {String? parent, bool media = false, bool sensitive = false}) =>
    mastodonPostFromStatus({
      ...
      'url': 'https://studio.example/@maya/$id',
      ...
      'media_attachments': media
          ? [
              {'type': 'image', 'preview_url': 'https://review.example/$id.png'},
            ]
          : [],
    })!;
...
class ReviewSaved extends SavedTweetModel {
  ReviewSaved() {
    update([
      for (final post in [reviewPost('root'), reviewPost('image-0', media: true), reviewPost('a')])
        SavedTweet(
          id: mastodonArchiveId(post),
          user: post.acct,
          folderId: 'inspiration',
          note: post.id == 'root' ? 'Colours to come back to.' : null,
          content: jsonEncode(mastodonArchiveBlob(post)),
        ),
    ]);
  }
  @override
...
class ReviewFolders extends SavedTweetFolderModel {
  ReviewFolders() {
    update([SavedTweetFolder(id: 'inspiration', name: 'Inspiration', position: 0, createdAt: DateTime(2026, 9, 7))]);
  }
  @override
  Future<void> listFolders() async {}
}

class ReviewLikes extends LikedTweetModel {
  @override
  Future<void> listLikedTweets() async {}
}

class ReviewNotes extends LocalPostModel {
  @override
...
class ReviewGroups extends GroupsModel {
  ...
  @override
  Future<({int replies, int retweets})> countIncludeOverrides() async => (replies: 0, retweets: 0);
}

class ReaderReviewHarness extends MastodonHarness {
  final saved = ReviewSaved();
  final folders = ReviewFolders();
  final likes = ReviewLikes();
  ...
  Widget providers(Widget child) => MultiProvider(
    providers: [
      ...
      Provider<LocalPostModel>.value(value: notes),
...
Future<List<int>> reviewImageBytes() async {
  ...
  canvas.drawPath(
    Path()
      ...
      ..lineTo(0, 360)
      ..close(),
    Paint()..color = const Color(0xFF4B716E),
  );
  canvas.drawPath(
    Path()
      ..moveTo(0, 300)
      ..quadraticBezierTo(260, 240, 480, 330)
      ..lineTo(480, 360)
      ..lineTo(0, 360)
      ..close(),
    Paint()..color = const Color(0xFF223E44),
  );
...
class _ImageClient extends Fake implements HttpClient {
  ...
  @override
  Future<HttpClientRequest> openUrl(String method, Uri url) async => _ImageRequest(bytes);
  @override
