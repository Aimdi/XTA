...
void main() {
  ...
  group('replacePluginSlot', () {
    test('an empty write into an empty slot is not a change', () {
      final slots = <String, List<InterleavedItem>>{};

      expect(replacePluginSlot(slots, 'reddit', const []), isFalse);
      expect(slots.containsKey('reddit'), isFalse);
    });

    test('the first posts fill the slot', () {
      final slots = <String, List<InterleavedItem>>{};
      final items = [_item(_at(5))];

      expect(replacePluginSlot(slots, 'reddit', items), isTrue);
      expect(slots['reddit'], items);
    });

    test('clearing a filled slot is a change', () {
      final slots = <String, List<InterleavedItem>>{
        'reddit': [_item(_at(5))],
      };

      expect(replacePluginSlot(slots, 'reddit', const []), isTrue);
      expect(slots['reddit'], isEmpty);
    });
  });
}
