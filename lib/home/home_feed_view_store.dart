...
class HomeFeedViewState {
  ...
  HomeFeedViewState copyWith({String? sourceId, int? followingEpoch, int? forYouEpoch, int? stripEpoch}) =>
    HomeFeedViewState(sourceId: sourceId ?? this.sourceId, followingEpoch: followingEpoch ?? this.followingEpoch,
      forYouEpoch: forYouEpoch ?? this.forYouEpoch, stripEpoch: stripEpoch ?? this.stripEpoch);
}
...
class HomeFeedViewStore extends Store<HomeFeedViewState> {
  ...
  void selectSource(String id, {bool external = false}) => update(state.copyWith(
    sourceId: id, stripEpoch: state.stripEpoch + (external ? 1 : 0)));
