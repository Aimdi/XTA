...
Future<HomeTimelineSelection?> showHomeSourcePicker(
  ...
  final picked = await showModalBottomSheet<HomeTimelineSelection>(
    ...
    builder: (_) => _PickerSources(
      ...
      child: ScopedBuilder<FeedStripStore, List<String>>(
        store: strip,
        onState: (context, pins) =>
            ScopedBuilder<GroupsModel, List<SubscriptionGroup>>(
              store: groups,
              onState: (context, items) => GroupUnreadScope(
                builder: (context, unread) => AltMicrobloggingScope(
                  builder: (context, grouped) => HomeTimelinePicker(
                    ...
                    rememberedMicroblog: prefs.get<String>(
                      optionAltMicrobloggingLastSource,
                    ),
                    options: [
                      for (final source in availableFeedTabsFromIds(
                        pins,
                        prefs,
                      ))
                        HomeTimelineOption(
  ...
  return context.mounted && added != null
      ? HomeTimelineSelection.source(added)
      : null;
}
...
class _PickerSources extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget result = child;
    if (unread != null)
      result = Provider<GroupUnreadStore>.value(value: unread!, child: result);
    if (grouping != null)
      result = Provider<AltMicrobloggingStore>.value(
        value: grouping!,
        child: result,
      );
    return result;
