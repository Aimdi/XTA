...
List<OfflineMediaSource> offlineMediaOf(SavedContent content) {
  ...
  return [for (final media in attachments)
    if (media.type == 'photo' && (media.mediaUrlHttps ?? media.mediaUrl) != null)
      ...
      ..._video(media.videoInfo!.variants!.where((v) => v.contentType == 'video/mp4')
        .map((v) => (url: v.url, bitrate: v.bitrate ?? 0)).toList()),
  ];
}
...
bool offlineSavedSensitive(SavedContent content) => content.tweet?.possiblySensitive == true ||
  content.mastodon?.sensitive == true || (content.mastodon?.spoilerText.isNotEmpty ?? false) ||
  content.reddit?.over18 == true || content.reddit?.spoiler == true;

String offlineSavedTitle(SavedContent content) {
  final title = content.reddit?.title ?? content.mastodon?.text ??
    content.tweet?.fullText ?? content.tweet?.text ?? '';
  final plain = html.parseFragment(title).text ?? '';
  return plain.length > 300 ? '${plain.substring(0, 300)}…' : plain;
}

String offlineSavedSource(SavedContent content) => content.mastodon?.acct ??
  content.reddit?.subreddit ?? content.tweet?.user?.screenName ?? '';
