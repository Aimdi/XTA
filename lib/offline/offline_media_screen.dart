...
class _OfflineMediaScreenState extends State<OfflineMediaScreen> {
  ...
  @override
  void dispose() { _page.destroy(); super.dispose(); }

  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: Text(widget.entry.title, maxLines: 1, overflow: TextOverflow.ellipsis)),
    body: SafeArea(top: false, child: ScopedBuilder<_OfflinePageStore, int>(store: _page,
      onState: (context, page) => PageView.builder(itemCount: widget.entry.files.length,
      onPageChanged: _page.select,
      itemBuilder: (context, index) => Semantics(label: L10n.of(context).offline_media_label(index + 1),
        child: _OfflineMediaPage(key: ValueKey(widget.entry.files[index].name), active: index == page,
          file: widget.entry.files[index], entry: widget.entry, store: widget.store)))),
    ),
  );
}
...
class _OfflineMediaPage extends StatefulWidget {
  ...
  final OfflineStore store;
  final bool active;
  const _OfflineMediaPage({super.key, required this.file, required this.entry, required this.store, required this.active});
  @override
...
class _OfflineMediaPageState extends State<_OfflineMediaPage> with WidgetsBindingObserver {
  ...
  @override
  Widget build(BuildContext context) => FutureBuilder<File?>(future: _file,
    builder: (context, snapshot) {
      ...
      return InteractiveViewer(minScale: 1, maxScale: 5, child: Center(child: Image.file(file,
        fit: BoxFit.contain, errorBuilder: (context, _, _) => Text(L10n.of(context).offline_unavailable))));
    });
}
