...
class OfflineFile {
  ...
  factory OfflineFile.fromJson(Map<String, dynamic> json) => OfflineFile(
    ...
    mime: json['mime'] as String, bytes: json['bytes'] as int);
}
...
class OfflineEntry {
  ...
  const OfflineEntry({required this.id, required this.directory, required this.title,
    required this.source, required this.kind, required this.totalMedia,
    required this.files, required this.retainedAt, this.url, this.articleBytes = 0,
  ...
  OfflineEntry verified({required int articleBytes, required List<OfflineFile> files}) => OfflineEntry(
    id: id, directory: directory, title: title, source: source, kind: kind,
    totalMedia: totalMedia, files: List.unmodifiable(files), retainedAt: retainedAt,
    url: url, articleBytes: articleBytes, sensitive: sensitive);

  Map<String, dynamic> toJson() => {'id': id, 'directory': directory, 'title': title,
    'source': source, 'kind': kind.name, 'totalMedia': totalMedia,
    'files': files.map((f) => f.toJson()).toList(), 'retainedAt': retainedAt.toIso8601String(),
    'url': url, 'articleBytes': articleBytes, 'sensitive': sensitive};

  factory OfflineEntry.fromJson(Map<String, dynamic> json) => OfflineEntry(
    id: json['id'] as String, directory: json['directory'] as String,
    title: json['title'] as String, source: json['source'] as String,
    ...
    retainedAt: DateTime.parse(json['retainedAt'] as String), url: json['url'] as String?,
    articleBytes: json['articleBytes'] as int? ?? 0, sensitive: json['sensitive'] == true);
}
...
class OfflineState {
  ...
  const OfflineState({this.entries = const [], this.busy = const {},
    this.failed = const {}, this.loading = true, this.storageError = false});
  int get bytes => entries.fold(0, (total, entry) => total + entry.bytes);
  OfflineEntry? entry(String id) {
    for (final entry in entries) { if (entry.id == id) return entry; }
    return null;
  }
}
