...
class OfflineLibraryScreen extends StatelessWidget {
  ...
  Future<void> _clear(BuildContext context, OfflineStore model) async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      title: Text(l10n.offline_clear_question), content: Text(l10n.offline_clear_description),
  ...
  Future<void> _open(BuildContext context, OfflineStore model, OfflineEntry entry) async {
    if (entry.kind == OfflineKind.media) {
      ...
      await Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => OfflineMediaScreen(entry: entry, store: model)));
      return;
    }
  ...
  Future<bool> _reveal(BuildContext context) async {
    ...
    return await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      title: Text(l10n.offline_sensitive_open), content: Text(l10n.offline_sensitive_description),
      actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: Text(l10n.cancel)),
  ...
  Widget build(BuildContext context) {
    ...
    model.load();
    final l10n = L10n.of(context);
    return ScopedBuilder<OfflineStore, OfflineState>(store: model, onState: (context, state) => Scaffold(
      appBar: AppBar(title: Text(l10n.offline_library_title), actions: [
        IconButton(icon: const Icon(Icons.refresh), tooltip: l10n.retry, onPressed: state.loading ? null : model.refresh),
        IconButton(icon: const Icon(Icons.delete_sweep_outlined), tooltip: l10n.offline_clear,
          onPressed: state.entries.isEmpty && state.busy.isEmpty ? null : () => _clear(context, model)),
      ]),
      body: SafeArea(top: false, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 12), child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(l10n.offline_storage_used(offlineBytes(context, state.bytes), offlineBytes(context, model.storageLimit)),
              style: Theme.of(context).textTheme.titleSmall),
            const SizedBox(height: 8),
            LinearProgressIndicator(value: (state.bytes / model.storageLimit).clamp(0, 1).toDouble()),
            const SizedBox(height: 8),
            Text(l10n.offline_storage_limit(offlineBytes(context, model.storageLimit)), style: Theme.of(context).textTheme.bodySmall),
          ])),
        if (state.storageError) Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
        ...
        Expanded(child: state.loading ? const Center(child: CircularProgressIndicator())
          : state.entries.isEmpty ? Center(child: Padding(padding: const EdgeInsets.all(24),
              child: Text(l10n.offline_empty, textAlign: TextAlign.center)))
          : ListView.separated(itemCount: state.entries.length,
  ...
  Widget _entry(BuildContext context, OfflineStore model, OfflineEntry entry) {
    final l10n = L10n.of(context);
    return ListTile(contentPadding: const EdgeInsetsDirectional.fromSTEB(16, 8, 8, 8),
      leading: Icon(entry.sensitive ? Icons.visibility_off_outlined :
        entry.kind == OfflineKind.article ? Icons.article_outlined : Icons.perm_media_outlined),
