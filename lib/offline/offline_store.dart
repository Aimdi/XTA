...
class OfflineStore extends Store<OfflineState> {
  ...
  OfflineStore({
    ...
    http.Client? client,
    this.storageLimit = defaultStorageLimit,
    this.imageLimit = 8 * 1024 * 1024,
  ...
  void _emit({List<OfflineEntry>? entries, Set<String>? busy, Set<String>? failed, bool? loading, bool? storageError}) {
    if (_disposed) return;
    update(
      OfflineState(
        entries: List.unmodifiable(entries ?? state.entries),
        busy: Set.unmodifiable(busy ?? state.busy),
        failed: Set.unmodifiable(failed ?? state.failed),
        loading: loading ?? state.loading,
        storageError: storageError ?? state.storageError,
      ),
    );
  }
      ...
      if (await manifest.exists()) {
        ...
        for (final value in raw) {
          try {
            ...
            /* A damaged row does not hide the rest of the library. */
          }
        }
      }
  ...
  bool _safeEntry(OfflineEntry entry) =>
      ...
      entry.id.isNotEmpty &&
      entry.id.length < 8192 &&
      entry.files.length <= maxMedia &&
      entry.totalMedia >= entry.files.length &&
      entry.totalMedia <= 10000 &&
      entry.articleBytes >= 0 &&
      entry.articleBytes <= maxArticleBytes &&
      entry.files.every(
        (f) => RegExp(r'^media-[0-9]+$').hasMatch(f.name) && f.bytes > 0 && f.bytes <= max(videoLimit, imageLimit),
      );
  ...
  Future<OfflineEntry> _verify(OfflineEntry entry) async {
    ...
    if (entry.articleBytes > 0 && await articleFile.exists() && await articleFile.length() == entry.articleBytes) {
      try {
        final article = OfflineArticle.fromJson(
          Map<String, dynamic>.from(jsonDecode(await articleFile.readAsString()) as Map),
        );
        if (article.id == entry.id && article.bodyHtml.trim().isNotEmpty) articleBytes = entry.articleBytes;
      } catch (_) {
        /* Only verified text is shown as retained. */
      }
    }
  ...
  Future<void> keepArticle(OfflineArticle article) => _keep(
    ...
    url: article.url,
    article: article,
    media: articleImageSources(article),
  );

  Future<void> keepMedia({
    required String id,
    required String title,
    required String source,
    required List<OfflineMediaSource> media,
    String? url,
  ...
  Future<void> _keep({
    ...
    return _queue(() async {
      ...
      try {
        ...
        if ((state.entries.length >= maxItems && state.entry(id) == null) || media.length > 10000)
          throw const FileSystemException('Offline library full');
        ...
        if (bodyBytes.length > maxArticleBytes ||
            bodyBytes.length > available ||
            (article != null && article.bodyHtml.trim().isEmpty))
          throw const FileSystemException('Article unavailable or too large');
        ...
        final entry = OfflineEntry(
          ...
          source: source,
          url: url,
          kind: article == null ? OfflineKind.media : OfflineKind.article,
          articleBytes: bodyBytes.length,
          totalMedia: media.length,
          files: List.unmodifiable(files),
          sensitive: sensitive,
          retainedAt: DateTime.now(),
        );
  ...
  Future<List<OfflineFile>> _retainFiles(
    Directory staging,
    List<OfflineMediaSource> media,
    int budget,
    String id,
    int generation,
  ) async {
    ...
    for (var i = 0; i < min(media.length, maxMedia); i++) {
      ...
      try {
        final file = await retainOfflineMedia(
          client: _client,
          source: media[i],
        ...
        /* Partial image availability is reported by the manifest. */
      }
    }
    return files;
  }
  ...
  Future<void> _reflectStorageFailure() async {
    ...
    for (final entry in state.entries) {
      try {
        actual.add(await _verify(entry));
      } catch (_) {
        actual.add(entry.verified(articleBytes: 0, files: []));
      }
    }
  ...
  Future<String> renderArticle(String id, String sanitizedDocument) async {
    ...
    for (final source in document.querySelectorAll('source,video,audio,iframe,object,embed')) {
      source.remove();
    }
...
String? resolveOfflineUrl(String? raw, String? base) {
  ...
  return resolved != null && ['http', 'https'].contains(resolved.scheme) && resolved.host.isNotEmpty
      ? resolved.toString()
      : null;
}
...
List<OfflineMediaSource> articleImageSources(OfflineArticle article) {
  ...
  final urls = <String>{};
  final cover = resolveOfflineUrl(
    article.payload['imageUrl'] as String? ?? article.payload['cover_image'] as String?,
    article.url,
  );
