...
Future<void> addBlueskyAccountToGroup(BuildContext context, BlueskyAccount account) {
  ...
  return editPluginAccountGroups(context, subscription: bluesky.subscriptionOf(account),
    ensureFollowed: () async { if (!store.follows(account.handle)) await store.add(account); });
}
...
Future<void> addMastodonAccountToGroup(BuildContext context, MastodonAccount account) {
  ...
  return editPluginAccountGroups(context, subscription: mastodon.subscriptionOf(account),
    ensureFollowed: () async { if (!store.follows(account.acct)) await store.add(account); });
}
...
Future<void> addThreadsAccountToGroup(BuildContext context, ThreadsAccount account) {
  ...
  return editPluginAccountGroups(context, subscription: threads.subscriptionOf(account),
    ensureFollowed: () async { if (!store.follows(account.handle)) await store.add(account); });
}
