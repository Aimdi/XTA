...
class BlueskyMediaGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SliverPadding(
      ...
      sliver: SliverLayoutBuilder(builder: (context, constraints) => SliverGrid.builder(
        ...
        child: InkWell(
          ...
          child: Stack(fit: StackFit.expand, children: [
            if (post.sensitive)
              Center(child: Padding(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(l10n.content_warning, textAlign: TextAlign.center,
                    maxLines: 2, overflow: TextOverflow.ellipsis,
                    style: Theme.of(context).textTheme.labelSmall),
                ]),
              ))
            else if (post.hasMedia)
              LayoutBuilder(builder: (context, constraints) => ExtendedImage.network(
                post.images.first, fit: BoxFit.cover,
                ...
                loadStateChanged: (state) => state.extendedImageLoadState == LoadState.failed
                    ? Icon(Icons.broken_image_outlined, color: colors.onSurfaceVariant) : null,
              )),
            ...
            if (post.images.length > 1)
              const PositionedDirectional(end: 6, top: 6,
                child: _MediaBadge(icon: Icons.collections_outlined)),
          ]),
        ),
      ),
    );
...
class _MediaBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DecoratedBox(
    ...
    child: Padding(padding: const EdgeInsets.all(5), child: Icon(icon, color: Colors.white, size: 20)),
  );
}
