...
class BlueskyProfileFeed {
  ...
  const BlueskyProfileFeed({this.posts = const [], this.cursor, this.loaded = false, this.loading = false, this.error});
}
...
class BlueskyProfileStore extends Store<BlueskyProfileState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      if (_closed || request != _profileRequest) return;
      update(BlueskyProfileState(profile: state.profile, selected: state.selected, feeds: state.feeds, error: error));
    }
  }
  ...
  Future<void> load(PluginProfileFeedTab tab, {bool more = false}) async {
    ...
    try {
      ...
      _setFeed(
        tab,
        BlueskyProfileFeed(posts: previous.posts, cursor: previous.cursor, loaded: previous.loaded, error: error),
      );
    }
  }
