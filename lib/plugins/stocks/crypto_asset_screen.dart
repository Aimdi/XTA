...
class _CryptoAssetScreenState extends State<CryptoAssetScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(asset.label), actions: [
      ...
      body: NestedScrollView(
        headerSliverBuilder: (context, _) => [SliverToBoxAdapter(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              ScopedBuilder<CryptoDetailStore, CryptoDetailState>(
                store: _market,
                onState: (_, state) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  if (state.loading) const LinearProgressIndicator(),
                  if (state.failed) ListTile(contentPadding: EdgeInsets.zero,
              ...
              Wrap(spacing: 8, children: [
                OutlinedButton.icon(
                  icon: const Icon(Icons.copy_outlined), label: Text(l10n.plugin_stocks_crypto_copy_contract),
                  onPressed: () => Clipboard.setData(ClipboardData(text: asset.address)),
                ),
                ScopedBuilder<CryptoDetailStore, CryptoDetailState>(
                  store: _market,
                  onState: (_, state) => OutlinedButton.icon(
                    icon: const Icon(Icons.open_in_new), label: Text(l10n.open_in_browser),
                    onPressed: state.market?.pairAddress == null ? null : () => openUri(context,
                      Uri.https('dexscreener.com', '/${asset.chain}/${state.market!.pairAddress}').toString()),
                  ),
                ),
              ]),
              const SizedBox(height: 12),
              Text(l10n.plugin_stocks_crypto_posts_hint,
  ...
  Widget _price(BuildContext context, CryptoMarket? market) {
    ...
    return Wrap(spacing: 12, crossAxisAlignment: WrapCrossAlignment.center, children: [
      Text(market?.price == null ? kStockPlaceholder : '${stockAssetPrice(market!.price!)} USD',
        style: Theme.of(context).textTheme.headlineMedium),
      if (percent != null) Text(stockPercentLabel(percent),
