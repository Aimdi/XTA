...
class StocksSearchState {
  ...
  const StocksSearchState({this.query = '', this.crypto = false, this.loading = false,
    this.failed = false, this.stocks = const [], this.tokens = const []});
}
...
class StocksSearchStore extends Store<StocksSearchState> {
  ...
  int _generation = 0;
  bool _closed = false;
  StocksSearchStore({required this.tickerClient, required this.cryptoClient})
      : super(const StocksSearchState());

  void change(String query, {bool? crypto, bool immediate = false}) {
    _debounce?.cancel();
    final generation = ++_generation;
    final isCrypto = crypto ?? state.crypto;
    update(StocksSearchState(query: query.trim(), crypto: isCrypto,
    ...
    if (immediate) {
      ...
      _debounce = Timer(const Duration(milliseconds: 300),
        () => _search(state.query, isCrypto, generation));
    }
  }
