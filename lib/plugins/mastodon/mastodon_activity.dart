...
String _activityError(L10n l10n, Object error) =>
    error is MastodonException &&
        (error.kind == MastodonErrorKind.unauthorized || error.kind == MastodonErrorKind.notFound)
    ? l10n.plugin_post_activity_unavailable
    : mastodonErrorMessage(l10n, error);
...
Future<void> openMastodonReposts(BuildContext context, MastodonPost post) {
  ...
  return openPluginActivity<MastodonProfile>(
    context,
    title: L10n.of(context).plugin_post_reposted_by,
    postUrl: post.url,
    loader: (cursor) => client.getRepostedBy(instances, post, cursor: cursor),
    idOf: (person) => person.acct,
...
Future<void> openMastodonQuotes(BuildContext context, MastodonPost post) {
  final client = context.read<MastodonClient>();
  final instances = _instances(context, post);
  return openPluginActivity<MastodonPost>(
    context,
    title: L10n.of(context).quotes,
    postUrl: post.url,
    loader: (cursor) => client.getQuotes(instances, post, cursor: cursor),
    idOf: (post) => post.url,
    errorLabel: _activityError,
