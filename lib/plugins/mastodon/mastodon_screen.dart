...
import 'package:xta/plugins/mastodon/mastodon_timeline_pane.dart';
...
class MastodonScreen extends StatefulWidget {
  ...
  const MastodonScreen({
    super.key,
    required this.scrollController,
    this.fullClient = false,
  });
...
class _MastodonScreenState extends State<MastodonScreen> {
  ...
  late final MastodonTimelineControlsStore _timelineControls;
  ...
  void initState() {
    ...
    _reading = _session.obtain(
      'reading',
      () => MastodonReadingStore(prefs, scope),
    );
    _timelineControls = _session.obtain(
      'timeline-controls',
      () => MastodonTimelineControlsStore(prefs),
    );
    _reading.changeScope(scope);
    _tabs = _session.obtain(
      'view',
      () => _MastodonTabStore(_reading.state.tab),
    );
    _people = _session.obtain(
      'people',
      () => PluginViewStore(_reading.state.people),
    );
    _pageStorage = _session.obtain(
      widget.fullClient ? 'client-scroll' : 'home-scroll',
  ...
  Future<void> _settings() async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const MastodonSettingsScreen()),
    );
    if (!mounted || previous == mastodonConfiguredInstances(prefs).join('|'))
      return;
      ...
      child: ScopedBuilder<_MastodonTabStore, int>(
        ...
        onState: (context, tab) => Scaffold(
          ...
          appBar: widget.fullClient
              ? MastodonClientBar(
                  selected: tab,
                  height: (MediaQuery.textScalerOf(context).scale(40) + 16)
                      .clamp(64, double.infinity),
                  onSearch: _lookUpProfile,
                  onSettings: _settings,
                )
              : null,
          bottomNavigationBar: widget.fullClient
              ? MastodonNavigation(selected: tab, onSelected: _onTab)
              : null,
  ...
  Widget _controls(BuildContext context, int tab) =>
      ScopedBuilder<PluginViewStore<bool>, bool>(
        store: _chrome,
        onState: (context, visible) {
          final child = widget.fullClient || visible
              ? Column(
                  ...
                  children: [
                    ...
                    if (tab == 3)
                      ScopedBuilder<PluginViewStore<bool>, bool>(
                        store: _people,
                        onState: (context, people) => MastodonFollowingControls(
                          people: people,
                          onSelected: _selectPeople,
                          onAdd: _addAccount,
                        ),
                      ),
                  ],
          ...
          if (widget.fullClient || MediaQuery.disableAnimationsOf(context))
            return child;
          return AnimatedSize(
  ...
  void _restoreReading(int index) {
    ...
    if (index == 0) {
      ...
      if (store.state.posts.isEmpty)
        store.update(MastodonExplorePage(posts: point.posts, tags: point.tags));
    } else if (index == 1) {
      context.read<MastodonLocalStore>().restoreReading(
        point.posts,
        point.instance,
      );
    } else if (index == 2) {
      context.read<MastodonFederatedStore>().restoreReading(
        point.posts,
        point.instance,
      );
    } else {
      ...
      if (store.state.isEmpty) {
        store.update(
          point.posts
              .where(
                (post) =>
                    accounts.follows(post.acct) ||
                    (post.boostedByAcct != null &&
                        accounts.follows(post.boostedByAcct!)),
              )
              .toList(),
        );
      }
  ...
  void _onTab(int index) {
    ...
    if (index == 0) {
      final store = context.read<MastodonExploreStore>();
      if (store.state.posts.isEmpty && store.state.tags.isEmpty)
...
class _ExplorePane extends StatelessWidget {
  ...
  Widget _exploreBody(
    ...
    return RefreshIndicator(
      onRefresh: context.read<MastodonExploreStore>().refresh,
      child: MastodonTimelinePane(
        slot: slot,
        controller: scrollController,
        posts: page.posts,
        tags: page.tags,
        refreshError: context.read<MastodonExploreStore>().refreshError,
        onRefresh: context.read<MastodonExploreStore>().refresh,
              ...
              itemBuilder: (context, index) => Center(
                child: ActionChip(
                  ...
                  onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => MastodonTagScreen(tag: tags[index].name),
                    ),
                  ),
                ),
              ),
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: RefreshIndicator(
        ...
        child: MastodonTimelinePane(
          ...
          onLoadMore: store.canLoadMore ? store.loadMore : null,
          onRetryMore: store.retryLoadMore,
  ...
  Widget _followingList(
    ...
    return RefreshIndicator(
      ...
      child: MastodonTimelinePane(
        ...
        controller: scrollController,
        posts: posts,
        refreshError: context.read<MastodonFeedStore>().refreshError,
        onRefresh: () => context.read<MastodonFeedStore>().refresh(force: true),
      ),
    );
  }
    ...
    return AlertDialog(
      ...
      content: Form(
        ...
        child: TextFormField(
          ...
          validator: (value) => normaliseMastodonAcct(value ?? '') == null
              ? l10n.plugin_mastodon_invalid_handle
              : null,
