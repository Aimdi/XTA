...
class MastodonReadingList extends StatefulWidget {
  ...
  const MastodonReadingList({super.key, required this.slot, required this.posts,
    required this.controller, this.heading, this.tags = const [], this.instance, this.loadingMore = false});
  @override
...
class _MastodonReadingListState extends State<MastodonReadingList> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (layout.position && _anchor != null && controller?.hasClients == true &&
          widget.posts.any((post) => post.url == _anchor)) {
        controller!.jumpTo((-_initial!.leading).clamp(controller.position.minScrollExtent, controller.position.maxScrollExtent));
      }
      _restoring = false;
      _remember();
    });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state != AppLifecycleState.resumed) { _remember(); _memory.flush(); }
  }
  ...
  void didUpdateWidget(covariant MastodonReadingList oldWidget) {
    ...
    if (oldWidget.posts != widget.posts) {
      WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _remember(); });
    }
  }
  ...
  void _remember() {
    ...
    for (var index = 0; index < widget.posts.length; index++) {
      ...
      if (y + box.size.height > 1 && y < viewport.size.height) { visible = index; leading = y; break; }
    }
    final start = (visible - 24).clamp(0, widget.posts.length);
    _memory.remember(widget.slot, MastodonReadPoint(
      posts: widget.posts.skip(start).take(mastodonReadingLimit).toList(),
      tags: widget.tags, anchor: widget.posts[visible].url, leading: leading, instance: widget.instance));
  }
  ...
  Widget _post(int index) {
    ...
    return KeyedSubtree(key: _rows.putIfAbsent(post.url, GlobalKey.new),
  ...
  Widget build(BuildContext context) {
    ...
    Widget tail() => const Padding(padding: EdgeInsets.all(16), child: Center(child: CircularProgressIndicator()));
    return NotificationListener<ScrollNotification>(
      onNotification: (notification) { if (notification is ScrollEndNotification && notification.depth == 0) _remember(); return false; },
      child: CustomScrollView(key: _viewport,
        controller: pluginInnerScrollController(context, widget.controller), primary: false,
        physics: const AlwaysScrollableScrollPhysics(), center: anchorIndex < 0 ? null : _center,
        slivers: [
          if (anchorIndex >= 0) SliverList.builder(itemCount: anchorIndex + prefix,
            itemBuilder: (context, index) {
              final before = anchorIndex - index - 1;
              return before < 0 ? widget.heading! : _post(before);
            }),
          SliverList.builder(key: anchorIndex < 0 ? null : _center,
            itemCount: anchorIndex < 0 ? widget.posts.length + prefix + footer : widget.posts.length - anchorIndex + footer,
            itemBuilder: (context, index) {
