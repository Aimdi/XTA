...
PluginAccountSubscription pixivSubscription(PixivUser user) =>
    PluginAccountSubscription(pluginIdPixiv, {
      'id': '${user.id}', 'screen_name': user.account, 'name': user.name,
      'avatar_url': user.avatarUrl, 'created_at': DateTime.now().toIso8601String(),
    });

List<PluginAccountSubscription> readPixivGroupSubscriptions(BasePrefService prefs) {
  Object? value;
  try { value = jsonDecode(prefs.get<String>(optionPluginPixivGroupSubscriptions) ?? '[]'); }
  on FormatException { return const []; }
  if (value is! List) return const [];
  return [for (final row in value.whereType<Map>())
...
class PixivGroupSubscriptionsStore extends Store<List<PluginAccountSubscription>> {
  ...
  Future<void> add(PixivUser user) async {
    ...
    await _save([for (final item in current) if (item.id != next.id) item, next]);
  }
