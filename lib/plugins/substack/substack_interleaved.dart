...
/// chains.
///
/// Publication reads are bounded to three in flight; the feed used to wait on the sum of the round
...
Future<List<InterleavedItem>> loadSubstackInterleaved(
  ...
  Object? failure;
  final fetched = await mapWithConcurrency(publications, 3, (publication) async {
    try {
      ...
      for (final post in posts)
        if (post.publishedAt case final date?)
          provenanceInterleavedItem(
            ...
            id: '$pluginIdSubstack:${substackFeedPostKey(post)}',
