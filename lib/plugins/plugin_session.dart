...
class PluginSessionStore extends Store<Map<String, Object>> {
  ...
  Future<void> destroy() async {
    for (final dispose in _disposers.reversed) { dispose(); }
    _disposers.clear();
    await super.destroy();
  }
}
...
class PluginSessionLease {
  ...
  PluginSessionLease(BuildContext context, this.pluginId)
      : _session = context.read<PluginSessionStore?>();

  T obtain<T extends Object>(String slot, T Function() create, {void Function(T)? dispose}) {
    VoidCallback disposer(T value) => () {
      if (dispose != null) { dispose(value); }
      else if (value is Store) { value.destroy(); }
    };
  ...
  void dispose() {
    for (final dispose in _localDisposers.reversed) { dispose(); }
