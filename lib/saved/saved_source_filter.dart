...
SavedSource savedSourceOf(SavedContent? content) => content?.mastodon != null ? SavedSource.mastodon
  : content?.reddit != null ? SavedSource.reddit : SavedSource.x;

bool matchesSavedSource(SavedContent? content, SavedSource source) =>
  source == SavedSource.all || savedSourceOf(content) == source;

bool savedContentHasMedia(SavedContent? content) => content?.mastodon?.hasMedia == true ||
...
class SavedSourceButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String label(SavedSource source) => switch (source) {
      SavedSource.all => l10n.home_networks_all, SavedSource.x => l10n.source_x,
      SavedSource.reddit => l10n.plugin_reddit_title, SavedSource.mastodon => l10n.plugin_mastodon_title,
    };
    return PopupMenuButton<SavedSource>(
      key: const ValueKey('saved-source-filter'),
      tooltip: l10n.home_networks, initialValue: selected, onSelected: onSelected,
      itemBuilder: (context) => [for (final source in SavedSource.values)
        CheckedPopupMenuItem(value: source, checked: source == selected, child: Text(label(source)))],
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(mainAxisSize: MainAxisSize.min,
        children: [Icon(Icons.public, size: 18, color: selected == SavedSource.all ? null : Theme.of(context).colorScheme.primary),
          const SizedBox(width: 6), Text(selected == SavedSource.all ? l10n.home_networks : label(selected)), const Icon(Icons.expand_more, size: 16)])),
    );
  }
}
