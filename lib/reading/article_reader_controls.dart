...
class ArticleReaderControls extends StatelessWidget {
  ...
  const ArticleReaderControls({super.key, required this.store,
    required this.onAppearanceChanged, required this.onStartOver, this.supportsAppearance = true, this.canComplete = true});
  ...
  Widget build(BuildContext context) => ScopedBuilder<ArticleReadingStore, ArticleReadingState>(
    ...
    onState: (context, state) {
      ...
      final percent = NumberFormat.percentPattern(Localizations.localeOf(context).toString()).format(state.point.fraction);
      return Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: Text(
              state.point.completed ? l10n.article_reader_finished :
                state.resumed ? '${l10n.article_reader_resumed} · $percent' :
                state.point.fraction > 0.01 ? percent : l10n.article_reader_opened,
              maxLines: 2,
            ...
            IconButton(
              ...
              onPressed: supportsAppearance ? () { store.startOver(); onStartOver(); } : null,
            ),
            IconButton(
              tooltip: l10n.article_reader_appearance,
              icon: const Icon(Icons.text_fields),
              onPressed: supportsAppearance ? () => showModalBottomSheet<void>(
    ...
    onState: (context, state) {
      ...
      return SingleChildScrollView(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
          ...
          Slider(value: state.fontSize, min: 16, max: 28, divisions: 12,
            label: state.fontSize.toStringAsFixed(0),
            semanticFormatterCallback: (value) => '${l10n.article_reader_text_size} ${value.round()}',
            onChanged: (value) { store.appearance(fontSize: value); onChanged(); }),
          Text(l10n.article_reader_line_spacing),
          Slider(value: state.lineHeight, min: 1.4, max: 2.2, divisions: 8,
            ...
            onChanged: (value) { store.appearance(lineHeight: value); onChanged(); }),
        ]),
      ));
    },
  );
