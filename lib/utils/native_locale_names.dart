/// The name each locale calls itself, e.g. `de` → "Deutsch".
///
/// Vendored from `flutter_localized_locales` 2.0.5 rather than depended on.
/// That package declares its whole `data/` directory as Flutter assets — 17 MB
/// of per-locale translated name tables — and every byte shipped in the APK.
/// This app only ever read the const map below, which needs no assets at all.
///
/// Copyright (c) 2019 Mykola Pokhylets. MIT licence; the full text is at
/// https://github.com/guidezpl/flutter-localized-locales/blob/master/LICENSE
///
/// [nativeLocaleNames] is generated upstream by `tool/build.sh` from CLDR — take
/// a new copy from the package rather than editing it by hand. Anything CLDR
/// does not carry goes in [_ourNames] below, and [nativeLocaleNameOf] is what
/// callers should ask.
library;

/// Names for locales CLDR has no entry for, so the picker has something to show
/// besides the code itself.
const _ourNames = <String, String>{
  // CLDR names `be` and `be_BY` but not the Latin-script Belarusian this app is
  // translated into, so the language list read "be_Latn" to the very readers
  // who chose it.
  'be_Latn': 'biełaruskaja (łacinka)',
};

/// What to call [code] in its own language, or null when nothing here knows.
String? nativeLocaleNameOf(String code) => _ourNames[code] ?? nativeLocaleNames[code];
