...
List<DiscoveryLoad> groupDiscoverySources(BuildContext context, Iterable<Subscription> members) {
  final prefs = PrefService.of(context, listen: false);
  ...
  final bluesky = enabled(pluginIdBluesky)
      ? members.whereType<BlueskySubscription>().map((e) => e.id).toList()
      : <String>[];
  final mastodon = enabled(pluginIdMastodon)
      ? members.whereType<MastodonSubscription>().map((e) => e.id).toList()
      : <String>[];
  ...
  return [
    if (x.isNotEmpty) () async => xDiscoveryAccounts(await _xDiscoveryCache.merge(x, _loadXAuthor, maxFetches: 6)),
    if (skyStore != null) () async => blueskyDiscoveryAccounts(await skyStore.postsFor(bluesky)),
    if (mastodonStore != null)
      () => _loadMastodonDiscovery(mastodonStore.client, mastodon, mastodonConfiguredInstances(prefs)),
        ...
        if (shared != null)
          if (shared.user?.idStr case final String id)
            DiscoveryAccount(
              ...
              date: shared.createdAt,
              supportingPost: shared,
            ),
];

Future<List<DiscoveryAccount>> loadPixivDiscovery(PixivClient client, List<int> userIds, PixivMuteState mute) async {
