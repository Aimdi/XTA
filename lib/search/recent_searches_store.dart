...
class RecentSearchesStore extends Store<Map<String, List<String>>> {
  ...
  RecentSearchesStore(this.prefs) : super(const {}) {
    update(_read());
  }
  ...
  Map<String, List<String>> _read() {
    try {
      ...
      return {
        for (final entry in decoded.entries.take(24))
          if (entry.key is String && entry.value is List)
            entry.key as String: (entry.value as List)
                .whereType<String>()
                .where((q) => q.isNotEmpty && q.length <= 200)
                .take(10)
                .toList(),
      };
    } catch (_) {
  ...
  Future<void> remember(String scope, String value) {
    ...
    return _change(
      scope,
      (previous) => [query, ...previous.where((old) => old.toLowerCase() != query.toLowerCase())].take(10).toList(),
    );
  }
  ...
  Future<void> clear(String scope) => _change(scope, (_) => const []);
